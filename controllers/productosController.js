...
export const create = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ mensaje: "Error al crear Productos", error });
  }
};
...
export const update = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ mensaje: "Error al actualizar Productos", error });
  }
};
...
export const remove = async (req, res) => {
  try {
    ...
    if (!item) return res.status(404).json({ mensaje: "No encontrado" });
    await item.destroy();
    res.json({ mensaje: "Productos eliminado correctamente" });
  } catch (error) {
    console.error(error);
